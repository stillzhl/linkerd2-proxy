...
RUN --mount=type=cache,target=target \
  ...
  (cd linkerd2-proxy && /usr/bin/time -v cargo build --locked --target=x86_64-unknown-linux-musl --features="$PROXY_FEATURES") && \
  mv target/debug/linkerd2-proxy /out/linkerd2-proxy ; \
  else \
  (cd linkerd2-proxy && /usr/bin/time -v cargo build --locked --release=x86_64-unknown-linux-musl --features="$PROXY_FEATURES") && \
