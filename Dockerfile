...
ARG RUST_IMAGE=rust:1.47.0-buster
...
RUN --mount=type=cache,from=rust:1.47.0-buster,source=/usr/local/rustup,target=/usr/local/rustup \
  rustup target add x86_64-unknown-linux-musl

RUN --mount=type=cache,target=/var/lib/apt/lists \
  --mount=type=cache,target=/var/tmp \
  apt update && apt install -y time cmake musl-tools

WORKDIR /usr/src/linkerd2-proxy
COPY . .
RUN --mount=type=cache,target=target \
  --mount=type=cache,from=rust:1.47.0-buster,source=/usr/local/cargo,target=/usr/local/cargo \
  mkdir -p /out && \
  if [ -n "$PROXY_UNOPTIMIZED" ]; then \
  (cd linkerd2-proxy && /usr/bin/time -v cargo build --locked --target=x86_64-unknown-linux-$GLIBC --features="$PROXY_FEATURES") && \
  mv target/debug/linkerd2-proxy /out/linkerd2-proxy ; \
  else \
  (cd linkerd2-proxy && /usr/bin/time -v cargo build --locked --release=x86_64-unknown-linux-$GLIBC --features="$PROXY_FEATURES") && \
