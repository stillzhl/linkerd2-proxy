...
impl Config {
        ...
        P::Future: Send,
    {
        ...
        let http = {
            let router = http_router(&self, connect, profiles, tap, &metrics, traces.clone());
            http_server(&self.proxy, router, &metrics, traces, drain)
        };
    ...
    P::Error: Send,
{
    ...
    let target = endpoint
        ...
        .push_on_response(TraceContext::layer(
            traces.map(|traces| SpanConverter::client(traces, trace_labels())),
        ))
    ...
    let profile = target
        ...
        .push_when_unready(target.clone(), config.profile_idle_timeout)
        .check_new_service::<Target, http::Request<http::BoxBody>>();
    ...
    svc::stack(http)
        ...
        .push_on_response(
            svc::layers()
                ...
                .push(svc::ConcurrencyLimit::layer(config.max_in_flight_requests))
                // Eagerly fail requests when the proxy is out of capacity for a
                // dispatch_timeout.
                .push(svc::FailFast::layer("HTTP Server", config.dispatch_timeout))
                .push(metrics.http_errors.clone())
                // Synthesizes responses for proxy errors.
                .push(errors::layer())
                .push(TraceContext::layer(
                    traces.map(|traces| SpanConverter::server(traces, trace_labels())),
                ))
        ...
        .push(http::NewServeHttp::layer(config.server.h2_settings, drain))
